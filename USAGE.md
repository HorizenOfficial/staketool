...
VERSION
  staketool v1.1.1   linux-x64   node v12.18.1
...
NOTES
  ...
  If -txid is used, it is assumed that the transaction has already been broadcasted to the network.                                                                                                                               
                                                                                                                                                                                                                                  
  When this step completes the transaction id (txid) is written back to the verification tracking file. If "sendtxandstakeverification" is run again and the txid is present, the tool will check the tracking server for status. 
                                                                                                                                                                                                                                  
  The tracking server replies with whether the stake verification request is verified, needs to be confirmed, rejected, or if there is an error.                                                                                  
  The tracking system waits for a number of block conformations before changing the status to verified.                                                                                                                           
  The current status can be found by using the liststakes command or by checking the tracking server web site after setting up your API key.                                                                                      
                                                                                                                                                                                                                                  
  There is a time limit of 24 hours in which to complete a verification process. After 24 hours, "sendtxandstakeverification" will need to be repeated.                                                                           
...
VERSION
  signtxtool v1.1.1   linux-x64   node v12.18.1
